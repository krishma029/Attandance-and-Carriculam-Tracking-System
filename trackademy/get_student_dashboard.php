...
// Expecting POST parameters: enrollment_no (and optionally password)
...
try {
    $stmt = $conn->prepare("
        SELECT class_name, date, subject, topic, reference_link, status, student_name
    ...
    foreach ($records as $r) {
        if (substr($r['date'], 0, 10) === $today) { // ignore time
            $tasks[] = [
                "status" => $r['status'],
                "subject" => $r['subject'],
                "topic" => $r['topic'],
